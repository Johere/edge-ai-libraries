...
class DecordVideoDecoder(BaseVideoDecoder):
    def __init__(self, video_path: str, sample_fps: float, longest_side_size: Optional[int] = None):
        ...
        with vr_lock:
            self.vr = self._robust_video_reader(self._video_path, ctx=cpu(0), num_threads=DECORD_NUM_THREADS)
    ...
    def _robust_video_reader(url, ctx=cpu(0), num_threads=0, verify_ssl=True):
        """
        ...
        if not urlparse(url).scheme in ['https']:
            return VideoReader(url, ctx=ctx, num_threads=num_threads)
