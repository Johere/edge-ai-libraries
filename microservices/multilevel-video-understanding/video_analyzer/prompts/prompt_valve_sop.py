...
## 角色设定
请用气门间隙检测工作站的专业视角分析内容。接收一段视频的各个子片段文本总结, 严格对照“标准步骤集”分析，首先总结并分时间戳描述, 然后进行合规性判别。
用户要求: {question}
...
**工步3.** 
- 动作简述: 塞尺检查。
- 工艺控制点: 塞尺与三个进气侧气门交叉并停留, 塞尺检查共完成三次, 三次位置对应三个进气侧气门间隙
- 工步依赖: 无

**工步4-6.** 
- 动作简述: 如果**工步3**检查发现问题, 则调整螺母, 否则跳过
- 工艺控制点: 调整螺母, 并进行至少一次塞尺检查
- 工步依赖: 工步3
...
**工步9.** 
- 动作简述: 塞尺检查
- 工艺控制点: 塞尺与三个排气侧气门交叉并停留, 塞尺检查共完成三次, 三次位置对应三个排气侧气门间隙
- 工步依赖: 工步8

**工步10-13.** 
- 动作简述: 如果**工步9**检查发现问题, 则调整螺母, 否则跳过
- 工艺控制点: 调整螺母, 进行至少一次塞尺检查
- 工步依赖: 工步9

**工步14.** 
- 动作简述: 画线标记
- 工艺控制点: 白漆笔画线累积三次, 三次位置对应三个排气侧气门间隙
- 工步依赖: 工步9
...
**Guidelines**: 
请严格遵循以下步骤进行分析: 
1. **描述**: 先按照时间戳详细描述视频的各个子片段文本总结。“标准步骤集”中的工步描述仅作为判别依据，请参照实际视频内容进行分析，请勿在实际操作描述中将未发生的操作按照以下标准进行编造。
2. **判别**: 对照“标准步骤集”, 逐一判别每个工步是否被执行。
3. **输出格式**: 请务必包含“实际操作描述“和”工步执行判别结论“两个部分, 判别结果必须是**True**或者**False**, 并给出理由。
...
## 待总结内容:
以下是视频各子部分的总结, 每个子部分用分隔符 ">|<" 分开, 每个子部分总结会以其相对于完整视频的起止时间开头。
'''
...
**Guidelines**: 
请严格遵循以下步骤进行分析: 
1. **描述**: 按照时间戳详细描述视频中的实际画面。请参照实际视频内容进行分析，请勿编造。
2. **输出格式**: 输出“实际操作描述“。
...
### 角色设定
请用气门间隙检测工作站的专业视角分析视频内容。接收一段视频片段，该视频可能是一段完整视频，也可能是来自某个完整视频的片段, 请分析并生成详细的时间戳描述。
用户要求: {question}
...
if __name__ == "__main__":
    ...
    global_kwargs = {
        "question": "请判定视频是否完整执行进气、排气侧的检查流程",
    }

    macro_kwargs = {
        "question": "请概括这一时间段的关键动作",
        "st_tm": 12,
        "end_tm": 42,
    }

    local_kwargs = {
        "question": "结合标注实体说明技师当前的操作",
        "st_tm": 120,
        "end_tm": 150,
    }
    ...
    tminus_kwargs = {
        ...
        "st_tm": 105,
        "end_tm": 120,
        "past_summary": "技师完成了进气侧的塞尺检查并开始画线标记。",
    }

    print("=== GLOBAL PROMPT ===\n")
    print(prompter.assign_global_prompt(**global_kwargs))

    print("\n=== MACRO PROMPT ===\n")
    print(prompter.assign_macro_prompt(**macro_kwargs))

    print("\n=== LOCAL PROMPT (base64 redacted) ===\n")
    print(redact_base64(prompter.assign_local_prompt(**local_kwargs)))

    print("\n=== T-MINUS PROMPT ===\n")
    print(prompter.assign_t_minus_prompt(**tminus_kwargs))
